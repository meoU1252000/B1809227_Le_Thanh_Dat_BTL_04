...
if (!isset($_SESSION['admin'])) {
    ...
    $result = mysqli_query($conn, "SELECT * from nhanvien ORDER BY msnv ASC LIMIT $perRow,$rowsPerPage");
    ...
    for($i=1;$i<=$totalPages;$i++){
        if($page ==$i){
            $previous = '<a c href="index.php?page_layout=nhanvien&page='.($page-1).'">&laquo;</a>';
            $listPage.='<a class="active" href="index.php?page_layout=nhanvien&page='.$i.'">'.$i.'</a>';
            $nextPage = '<a  href="index.php?page_layout=nhanvien&page='.($page+1).'">&raquo;</a>';
        } else{
            $listPage.='<a  href="index.php?page_layout=nhanvien&page='.$i.'">'.$i.'</a>';
        }
    }
    mysqli_close($conn);
...
?>
  <script>
        ...
        function getSearch(a){
          ...
          null==xmlhttp?alert("Trình duyệt không hỗ trợ HTTP Request"):(xmlhttp.onreadystatechange=stateChanged,xmlhttp.open("GET","getDataNV.php?key="+a,!0),xmlhttp.send(null))
        }
                ...
                <div class="body-page--manage_form">
                    <table class="body-page--manage_form--table">
                        <tr class="body-page--manage_form--table-display">
                            ...
                            <th class="body-page--manage_form--table-col_position">
                                Chức Vụ
                            </th>
                        ...
                        <tr class="body-page--manage_form--table-tr">
                            ...
                            <td class="body-page--manage_form--table-col_action">
                                <div class="body-page--manage_form--table-col_action-form">
                                    <i class="far fa-edit edit-icon_action"><a href="index.php?page_layout=suanhanvien&id_nv=<?php echo $row['msnv'];?>" class ="action_edit">Sửa</a></i>
                                    <i class="far fa-trash-alt delete-icon_action"><a href="xoanhanvien.php?id_nv=<?php echo $row['msnv']; ?>" onClick="return xoaNhanVien();" class = "action_delete">Xóa</a></i>
                                </div>
                            </td>
                        </tr>
                        <?php 

...
include('../../admin/config.php');
  $key=$_GET['key'];
  if($key!=NULL){
      $sql="SELECT * from hanghoa where tenhh like '%$key%' and soluonghang >0 order by mshh ASC limit 10";
      ...
      echo $rs;
        ...
        $result = mysqli_query($conn, "SELECT * from HangHoa where soluonghang >0  ORDER BY mshh ASC LIMIT 10");
        $rs='';
        foreach($result as $row){
            $giamoi = $row['gia'] - ($row['gia'] * $row['giamgia']);
            $row['giamgia'] = $row['giamgia']*100;
